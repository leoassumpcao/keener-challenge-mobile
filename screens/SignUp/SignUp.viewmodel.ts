...
class SignUpViewModel {
  ...
  @observable
  private _arePasswordsMatching: boolean = true;
  @computed get arePasswordsMatching(): boolean {
    return this._arePasswordsMatching;
  }
  set arePasswordsMatching(value: boolean) {
    this._arePasswordsMatching = value;
  }
  ...
  @action onPasswordTextChange = (text: string): void => {
    ...
    else this.arePasswordsMatching = true;
  };
  ...
  @action signUp = async (): Promise<void> => {
    try {
      await this.register();
    } catch (e) {
      alert(e);
  ...
  async register(): Promise<void> {
    ...
    try {
      ...
      if (!this.email || !validator.isEmail(this.email)) {
        ...
        this.errors.push(
          "Please a valid birth date. e.g.: DD/MM/YYYY = " + this.birthDate
        );
